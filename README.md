# Deep reinforcement learning for a mean-field economy

This project is based on the author's 2020 thesis ***A Reinforcement Learning Approach to Krusell and Smith (1998) from the Perspective of Mean Field Game*** and applies deep reinforcement learning to compute an approximate equilibrium of the **Krusell and Smith (1998) model**. The model studies how household saving under uninsured employment risk and common productivity shocks shapes the wealth distribution and aggregate economic dynamics.
...
**Main finding:** DDPG yields results close to the dynamic-programming benchmark in this project’s baseline experiment ($`\beta=0.8`$). After five outer rounds, mean discounted utility is **8.8478**, compared with **8.9583** for dynamic programming—a relative utility-score gap of about **1.23%**.
...
The actor uses widths **4 → 200 → 100 → 1**, ReLU hidden activations, and output $`\mu_\theta(s)=\mathrm{clip}(h_\theta(s),0,2)/2`$. The critic has a **4 → 200 → 100** state branch and **1 → 100** action branch; their outputs are added, passed through ReLU, and mapped to scalar $`Q_\phi(s,a)`$. Normalization layers learn affine parameters while keeping their running statistics fixed.
...
The repository contains the updated TensorFlow implementation and the PyTorch reproduction. The structure below reflects the uploaded files; directory entries group their contained source, configuration, data, or figure files.

```text
krusell-smith-ddpg/
├── README.md
├── .gitattributes                         # Preserve historical data bytes
├── .gitignore                            # Exclude environments and generated runs
├── docs/figures/                          # Two method diagrams and five result figures
├── reproduction/                         # PyTorch implementation
│   ├── README.md                         # Detailed usage
│   ├── src/ks_reproduction/               # Economy, DDPG, DP, simulation, and CLI
│   ├── configs/                          # thesis.toml, smoke.toml, robustness.toml
│   ├── data/benchmark/                   # DP arrays, checksums, and provenance
│   ├── targets/thesis_results.json        # Numerical targets and tolerances
│   ├── docs/LEGACY_COMPATIBILITY.md       # Numerical compatibility notes
│   ├── pyproject.toml                    # Runtime dependencies and CLI entry point
│   └── uv.lock                           # Locked dependency versions
└── reproduction-tensorflow/               # Updated TensorFlow 1.14 implementation
    ├── README.md                         # Detailed usage
    ├── src/thesis_tf/                    # Stage CLI, archive verifier, and TF code
    ├── configs/                          # thesis.json, thesis_round20.json, smoke.json
    ├── data/
    │   ├── README.md                     # Input provenance
    │   ├── benchmark/                    # DP reference arrays and checksums
    │   └── historical/                   # Checkpoints, evaluation samples, and histories
    ├── targets/thesis_results.json        # Published and archived numerical values
    ├── docs/LEGACY_COMPATIBILITY.md       # Historical behavior and known discrepancies
    ├── environment.yml                   # Python 3.7 environment definition
    └── requirements/legacy-py37.txt       # TensorFlow-era runtime dependencies
```

Both implementations include the inputs needed by their documented commands. Newly generated files are written to `reproduction/outputs/` or `reproduction-tensorflow/runs/`; these directories are created when needed and are not uploaded.
...
Use separate environments for the two implementations. Each setup block below starts from the repository root.
...
The editable installation uses the configuration, targets, and benchmark files in this checkout, so keep the checkout in place. The small `smoke.toml` profile runs training, evaluation, plotting, and structural verification; it does not reproduce the thesis-scale numerical results. To run the full baseline, use `ks-reproduce reproduce --config configs/thesis.toml` from `reproduction/`. Full experiments can take many hours.

For a locked environment, use `uv sync --frozen --python 3.13` inside `reproduction/`, then run CLI commands with the `uv run --frozen` prefix. See the [PyTorch README](reproduction/README.md) for stages, devices, resuming runs, and robustness configurations.

### Updated TensorFlow implementation

Use an x86-64 Linux or Intel macOS environment with Python 3.7 and TensorFlow 1.14:
...
The `verify` command audits the bundled historical data; it does not train or restore TensorFlow models. This audit can also run with current Python and NumPy without TensorFlow. TensorFlow training and checkpoint replay require the legacy environment above; TensorFlow 1.14 has no native Apple Silicon wheel.

See the [TensorFlow README](reproduction-tensorflow/README.md) for `train`, `dp`, `evaluate`, and `compare` commands, fresh-run output directories, and the round-20 configuration.
